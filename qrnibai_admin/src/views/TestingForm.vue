...
<template #bottom></template>
  ...
  <v-dialog  v-model="dialogshowoffice"  max-width="300">
  <v-card >
    <v-row >
      <v-col cols="12" class="mt-5">
        <h1 v-if="selectedControlNo">Control Number: {{ selectedControlNo }}</h1>
       <v-select label="Select Office" v-model="selectedoffice" :items="Opisdaw" class="mx-4">
    ...
    <v-card-actions class="d-flex justify-center mt-n7">  
      <v-btn color="green"  @click="dialogshowoffice = false">Assign</v-btn>
    </v-card-actions>
...
cancelDialog() {
  ...
  updateSelectedInfo() {
    const selectedNameData = this.namesData.find(nameData => nameData.Opisdaw === this.selectedOffice);
    if (selectedNameData) {
