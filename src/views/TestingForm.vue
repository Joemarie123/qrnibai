...
<select v-if="!item.columns.timescanned" @change="saveremarks(item)" v-model="item.raw.Remarks" class="elementobai mt-n2 " id="select-element">
  ...
  <option value="PM Shift">PM Shift</option>
        ...
        <tr v-for="(item, index) in selectedItems" :key="index">
          <td>{{ userData.office_id }}</td>
          <td>{{ item.value.event_id }}</td>
    ...
    <v-col cols="12" class="mt-n4 d-flex justify-end">
      <v-btn @click="saveAllData()">save</v-btn>
        <!-- <v-btn class="custom-btn rounded-lg"  @click="saveAllData()" >
...
fetchData() {
 ...
 name(decodedresult) {
  ...
  saveremarks(item){
   ...
   /*   this.selectedFullname = item.columns.fullname; */
  /*  if (item.raw.Remarks) {
      ...
      if (existingItemIndex !== -1) {
        ...
        this.selectedItems[existingItemIndex].value = { ...item.columns };
        this.selectedItems[existingItemIndex].value.Remarks = item.raw.Remarks;
      } else {
        // If not, add a new item
        this.selectedItems.push({
          value: { ...item.columns, Remarks: item.raw.Remarks },
        });
      }
